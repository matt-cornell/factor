...
impl LonLat {
    ...
    pub fn from_f64(lon: f64, lat: f64) -> Self {
        Self {
            lon: lon as _,
            lat: lat as _,
        }
    ...
    pub fn as_f64(self) -> (f64, f64) {
        ...
        (
            self.lon as f64 % TAU,
            (self.lat as f64).clamp(-FRAC_PI_2, FRAC_PI_2),
        )
    }
}
...
pub fn get_relative(start: LonLat, end: LonLat) -> Vec2 {
...
pub fn get_absolute(start: LonLat, offset: Vec2) -> LonLat {
    let LonLat { lon: lo1, lat: la1 } = start;
