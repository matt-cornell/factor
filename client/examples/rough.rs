...
fn mesh_quad(
    corners: [Vec2; 4],
    depth: usize,
    mut height: impl FnMut(usize, usize, Vec2) -> f32,
) -> Mesh {
    let len = depth + 1;
    let nverts = len * len;
    ...
    for i in 0..len {
        for j in 0..len {
            ...
            vertices[idx] = vert.extend(height(i, j, vert)).xzy();
    ...
    App::new()
        ...
        .add_systems(
            ...
            (
                ...
                render_axes.after(TransformSystem::TransformPropagate),
            ),
        )
        .run();
}
...
fn spawn_cell(
    ...
    let mesh = mesh_quad(corners, 32, |_, _, _| rand::random()).with_computed_normals();
    let size = Extent3d {
        width: 512,
        height: 512,
        ..default()
